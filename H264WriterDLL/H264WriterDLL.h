...
// version 0.5.0: Added HasH264() and HasHEVC(). Put back software encoding for testing in VMs. 
...
namespace H264WriterDLL {
	...
	public enum class Processing
	{
		HardwareAcceleration,
		Software
	};
	public enum class RateControlMode
	{
		None,
	...
	public: UINT32 getVideoFPS() { return m_VideoFPS; }
	public: void setVideoFPS(UINT32 val) {
			m_VideoFPS = val;
			m_FrameDuration = 10 * 1000 * 1000 / m_VideoFPS;
		}

	public: UINT32 getVideoBitrate() { return m_VideoBitrate; }
	public: void setVideoBitrate(UINT32 val) { m_VideoBitrate = val; }
	...
	public: int GetHeight() { return m_Height; }
		...
		H264Writer(String^ mp3_file, String^ dest_file, VideoCodec codec, Processing processing,
			...
			int quality/* Only valid when mode is RateControlMode::Quality. [0:100] 0 for smaller file size and less quality, 100 for bigger file size and more quality */)
		{
			...
			{
				throw gcnew System::SystemException(gcnew String(L"Error: MP3 filename is not set but duration is set to -1 meaning to follow MP3 duration!\nSet duration to a non-negative non-zero value!"));
			}
			m_Width = width;
			m_Height = height;
			m_cbWidth = 4 * m_Width;
			...
			m_VideoFPS = fps;
			m_FrameDuration = (10 * 1000 * 1000 / m_VideoFPS);
			m_pImage = gcnew array<System::UInt32>(m_Width * m_Height);
		...
		HRESULT InitializeWriter(DWORD* videoStreamIndex, DWORD* audioStreamIndex)
		{
			...
			*audioStreamIndex = 0;
			...
			DWORD           streamIndex = 1;
			IMFAttributes* pConfigAttrs = nullptr;
			...
			do
			{
				...
				if (String::IsNullOrEmpty(m_MP3Filename) == false)
				{
					// create a source reader
					pin_ptr<const wchar_t> mp3_file = PtrToStringChars(m_MP3Filename);
					IMFSourceReader* src_reader = m_pSourceReader;

					hr = MFCreateSourceReaderFromURL(mp3_file, pConfigAttrs, &src_reader);
					BREAK_ON_FAIL(hr);
					m_pSourceReader = src_reader;
			...
			const GUID& streamMajorType)
		{
			...
			IMFMediaType* pPartialMediaType = nullptr;
			IMFMediaType* pFullMediaType = nullptr;
		...
		{
			if (System::String::IsNullOrEmpty(m_MP3Filename))
				return S_OK;
		...
		)
		{
			...
			if (SUCCEEDED(hr))
			{
				pin_ptr<UInt32> image_data = &m_pImage[0];
				UInt32* image_ptr = image_data;
				hr = MFCopyImage(
					pData,              // Destination buffer.
					m_cbWidth,          // Destination stride.
					(BYTE*)image_ptr,    // First row in source image.
				...
				if (SUCCEEDED(hr))
				{
					...
					while (true)
					{
						...
						if (m_FrameRenderer)
						{
							...
							if (ret == false)
							{
								OutputDebugStringA("m_RenderFunction returned false!");
							}
						}
						else
						{
							OutputDebugStringA("m_RenderFunction is not set!");
							break;
						}
						...
						if (success)
						{
							...
							if (System::String::IsNullOrEmpty(m_MP3Filename) == false)
							{
